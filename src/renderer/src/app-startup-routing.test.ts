...
describe('renderer startup runtime routing', () => {
  it('loads settings before parallel repo hydration and worktrees after it', () => {
    ...
    expect(startupBlock.indexOf('actions.fetchRepos()', parallelHydrationIndex)).toBeLessThan(
      parallelHydrationEndIndex
    )
    expect(
      startupBlock.indexOf('actions.fetchProjectGroups()', parallelHydrationIndex)
    ).toBeLessThan(parallelHydrationEndIndex)
    expect(
      startupBlock.indexOf('actions.fetchFolderWorkspaces()', parallelHydrationIndex)
    ).toBeLessThan(parallelHydrationEndIndex)
